...
namespace BitboardCA
{
	...
	std::string GetCellString(unsigned int s)
	{
		switch ((int)s)
		{
			case 0:
	...
	{
		for ( unsigned int y = 0 ; y < viewable.GetSizeY() ; y ++ )
		{
			for ( unsigned int x = 0 ; x < viewable.GetSizeX() ; x ++ )
				(*m_pOstream) << GetCellString(viewable.GetCellState(x, y)).c_str() << " ";
			(*m_pOstream) << std::endl;
		}
	}

	void BitboardViewer::ViewBitboardLine(Bitboard b, unsigned int y)
	{
		b >>= y * 8;
	...
	void BitboardViewer::ViewLargeBitboard(Bitboard * bitboards, unsigned int size_x, unsigned int size_y)
	{
	...
	{
		unsigned int size_x = large_bitboard.GetBitboardListSizeX();
		unsigned int size_y = large_bitboard.GetBitboardListSizeY();
